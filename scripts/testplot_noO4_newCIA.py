...
def plotting(lamin, lamax, atmos, title):
    ...
    if atmos == 0: #0 = ocean loss
        ...
        ax.plot(wl, flux, label = " 10 bar Ocean Loss", color = 'orange')
        ax.plot(wl2, flux2, label = "10 bar Ocean Loss, no O$_2$-O$_2$", color = 'grey')
        ax.set_title(title)
        ...
        fig, ax = plt.subplots(figsize = (10,10))
        ax.plot(wl, flux, label = "10 bar Ocean Outgassing", color = 'orange')
        ax.plot(wl2, flux2, label = "10 bar Ocean Outgassing, no O$_2$-O$_2$", color = 'grey')
        ax.set_title(title)
        ax.set_ylabel("Reflectance")
        ax.set_xlabel("Wavelength ($\mu$m)")
        if lamin == 0.61:
